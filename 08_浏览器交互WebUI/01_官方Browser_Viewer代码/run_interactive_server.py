...
#   python run_interactive_server.py --bind 0.0.0.0         # listen address (default)
#
...
#   O                toggle auto-orbit
#   P                download canvas screenshot (PNG)
#   H                toggle HUD help
#   1-9              switch between loaded scenes
#   Scroll wheel     zoom forward/backward
#   Quality buttons  Low/Med/High JPEG over WebSocket
#
# Known limitations:
...
def main(args):
    ...
    async def handle_client(websocket):
        ...
        async def reader():
            nonlocal active_scene_idx, client_alive, jpeg_quality
            try:
                async for message in websocket:
                    ...
                    if msg["type"] == "pose":
                        ...
                        q = int(msg.get("jpeg_quality", jpeg_quality))
                        jpeg_quality = max(40, min(98, q))
        ...
        async def render_loop():
            ...
            while client_alive:
                ...
                t0 = time.time()
                print(f"  render start scene={sc['name']} ...", flush=True)
                ...
                print(f"  render done {render_time*1000:.0f}ms", flush=True)
                ...
                pending_jpeg = loop.run_in_executor(
                    jpeg_executor, _encode_jpeg_np, img_np, q
                )
                header = json.dumps(
                    {
                ...
                frame_count += 1
                # Critical: first frame must be flushed, otherwise clients that
                # gate on lastFrameDrawn deadlock (pose#1 rendered, never sent).
                ...
                if frame_count % 30 == 0:
                    fps = 30 / frame_time
                    print(f"  [{sc['name']}] {fps:.1f} render-fps", flush=True)
                    frame_time = 0.0
